	...
	<form action="forecreateOrder" method="post">
		...
		<div class="address">
			...
			<div>
				...
				<table class="addressTable">
					...
					<tr>
						<td>邮政编码</td>
						<td><input id="post" name="post" placeholder="如果您不清楚邮递区号，请填写000000"
							type="text"></td>
					</tr>
					<tr>
						<td>收货人姓名<span class="redStar">*</span></td>
						<td><input id="receiver" name="receiver" placeholder="长度不超过25个字符"
							type="text"></td>
					</tr>
					<tr>
					...
					<c:forEach items="${ois}" var="oi" varStatus="st">
						<tr class="orderItemTR">
							<td class="orderItemFirstTD"><img class="orderItemImg"
								src="img/book/${oi.book.id}.jpg"></td>
							<td class="orderItemProductInfo"><a
								href="forebook?bid=${oi.book.id}" class="orderItemProductLink">
									${oi.book.name} </a> <img src="img/site/creditcard.png"
								title="支持信用卡支付"> <img src="img/site/7day.png"
								title="消费者保障服务,承诺7天退货"> <img src="img/site/promise.png"
								title="消费者保障服务,承诺如实描述"></td>
							<td><c:if test="${fn:containsIgnoreCase(user.vip,'VIP')}">

									<span class="orderItemProductPrice">￥<fmt:formatNumber
											type="number" value="${oi.book.price*0.97}"
											minFractionDigits="2" /></span>
								</c:if> <c:if test="${! fn:containsIgnoreCase(user.vip,'VIP')}">

									<span class="orderItemProductPrice">￥<fmt:formatNumber
											type="number" value="${oi.book.price}" minFractionDigits="2" /></span>
								</c:if></td>
							<td><span class="orderItemProductNumber">${oi.number}</span>
							</td>
							<td><c:if test="${fn:containsIgnoreCase(user.vip,'VIP')}">

									<span class="orderItemUnitSum"> ￥<fmt:formatNumber
											type="number" value="${oi.number*oi.book.price*0.97}"
											minFractionDigits="2" />
									</span>
								</c:if> <c:if test="${! fn:containsIgnoreCase(user.vip,'VIP')}">

									<span class="orderItemUnitSum"> ￥<fmt:formatNumber
											type="number" value="${oi.number*oi.book.price}"
											minFractionDigits="2" />
									</span>
								</c:if></td>
		...
		<div class="orderItemTotalSumDiv">
			<div class="pull-right">
				<c:if test="${fn:containsIgnoreCase(user.vip,'VIP')}">

					<span>实付款：</span>
					<span class="orderItemTotalSumSpan">￥<fmt:formatNumber
							type="number" value="${total*0.97}" minFractionDigits="2" /></span>
				</c:if>
				<c:if test="${! fn:containsIgnoreCase(user.vip,'VIP')}">
