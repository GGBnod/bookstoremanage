...
<script>
		...
		$(".orderItemNumberSetting")
				.keyup(
						...
						function() {
							...
							var stock = $(
									"span.orderItemStock[bid=" + bid + "]")
									.text();
							var price = $(
									"span.orderItemPromotePrice[bid=" + bid
											+ "]").text();

							var num = $(
									".orderItemNumberSetting[bid=" + bid + "]")
									.val();
							num = parseInt(num);
							if (isNaN(num))
								num = 1;
	...
	function syncPrice(oiid, num, price) {
		...
		$(".cartProductItemSmallSumPrice[oiid=" + oiid + "]").html(
				...
				<c:forEach items="${ois }" var="oi">
					<tr oiid="${oi.id}" class="cartProductItemTR">
						<td><img selectit="false" oiid="${oi.id}"
							...
							class="cartProductImg" src="img/book/${oi.book.id}.jpg"></td>
						<td>
							<div class="cartProductLinkOutDiv">
								<a href="forebook?bid=${oi.book.id}" class="cartProductLink">${oi.book.name}</a>
						...
						<td><c:if test="${fn:containsIgnoreCase(user.vip,'VIP')}">
								<span class="cartProductItemOringalPrice">￥${oi.book.price}</span>
								<span class="cartProductItemPromotionPrice">￥${oi.book.price*0.97}</span>
							</c:if> <c:if test="${! fn:containsIgnoreCase(user.vip,'VIP')}">
								<span class="cartProductItemPromotionPrice">￥${oi.book.price}</span>
							</c:if></td>

						<td>
							<div class="cartProductChangeNumberDiv">
								<span class="hidden orderItemStock " bid="${oi.book.id}">${oi.book.stock}</span>
								<span class="hidden orderItemPromotePrice " bid="${oi.book.id}">${oi.book.price}</span>
								<a bid="${oi.book.id}" oiid="${oi.id}" class="numberMinus"
									href="#nowhere">-</a>
								<input bid="${oi.book.id}"
									oiid="${oi.id}" class="orderItemNumberSetting"
									autocomplete="off" value="${oi.number}">
								<a stock="${oi.book.stock}" oiid="${oi.id}" bid="${oi.book.id}"
						...
						<td><c:if test="${fn:containsIgnoreCase(user.vip,'VIP')}">
								<span class="cartProductItemSmallSumPrice" oiid="${oi.id}"
									bid="${oi.book.id}"> ￥<fmt:formatNumber type="number"
										value="${oi.book.price*oi.number*0.97}" minFractionDigits="2" />
								</span>
							</c:if> <c:if test="${! fn:containsIgnoreCase(user.vip,'VIP')}">
								<span class="cartProductItemSmallSumPrice" oiid="${oi.id}"
									bid="${oi.book.id}"> ￥<fmt:formatNumber type="number"
										value="${oi.book.price*oi.number}" minFractionDigits="2" />
								</span>
							</c:if></td>
