			...
			<table class="orderListItemTable" orderStatus="${o.status}"
				...
				<tr class="orderListItemFirstTR" >
					<td colspan="2"><b><fmt:formatDate value="${o.createDate}"
							...
							${o.orderCode} </span></td>
					<td colspan="2">VIP会员享9.7折</td>
					<td colspan="1">祝您购物愉快</div>
					</a></td>
					<td class="orderItemDeleteTD"><c:if
							test="${fn:containsIgnoreCase(o.status,'finish')}">
							<a class="deleteOrderLink" oid="${o.id}" href="#nowhere"> <span
				...
				<c:forEach items="${o.orderItems}" var="oi" varStatus="st">
					<tr class="orderItemProductInfoPartTR">
						<td class="orderItemProductInfoPartTD"><img width="80"
							height="80" src="img/book/${oi.book.id}.jpg"></td>
						<td class="orderItemProductInfoPartTD">
							<div class="orderListItemProductLinkOutDiv">
								<a href="forebook?bid=${oi.book.id}">${oi.book.name}</a>
						...
						<td class="orderItemProductInfoPartTD" width="100px"><c:if
								test="${fn:containsIgnoreCase(user.vip,'VIP')}">

								<div class="orderListItemProductOriginalPrice">
									￥
									<fmt:formatNumber type="number" value="${oi.book.price}"
										minFractionDigits="2" />
								</div>
								<div class="orderListItemProductPrice">
									￥
									<fmt:formatNumber type="number" value="${oi.book.price*0.97}"
										minFractionDigits="2" />
								</div>
						...
						<c:if test="${o.status=='waitReview' }">
						<td width="100px">
							<a href="forereview?oid=${o.id}&bid=${oi.bid}">
